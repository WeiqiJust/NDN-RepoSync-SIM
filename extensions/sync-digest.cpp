...
// Other options: VP_md2, EVP_md5, EVP_sha, EVP_sha1,
// EVP_sha256, EVP_dss, EVP_dss1, EVP_mdc2, EVP_ripemd160
...
typedef transform_iterator<hex_from_4_bit<std::vector<uint8_t>::const_iterator::value_type>,
                           transform_width<std::vector<uint8_t>::const_iterator, 4, 8,
                           std::vector<uint8_t>::const_iterator::value_type> > string_from_binary;
...
struct hex_to_4_bit
{
  ...
  CharType operator() (CharType ch) const
  {
    ...
    if (value == -1)
      BOOST_THROW_EXCEPTION(ns3::ndn::Error::DigestCalculationError() << errmsg_info_int((int)ch));

    return value;
  }
};

typedef transform_width<transform_iterator<hex_to_4_bit<std::string::const_iterator::value_type>,
                        std::string::const_iterator>, 8, 4> string_to_binary;
