 ...
 *
 *     NS_LOG=ndn.Consumer:ndn.RepoSyncDrop ./waf --run=ndn-sync-delay-loss
 */
...
{  
  //NS_LOG_UNCOND ("RxDrop at " << Simulator::Now ().GetSeconds ()<<" name = "<<p->GetName());  
  count++;
} 
...
{
  //double dropRate[4] = {0, 0.01, 0.05, 0.1};
    //for (int j = 0; j < 4; j++)
    //{
          std::ifstream op;
          ...
          std::cout<<"      dropRate = "<<0.2<<std::endl;
          ...
          for (int k = 0; k<linkSize; k++)
          {
             ...
             pppNetDevice[k].Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (em));  
             pppNetDevice[k].Get(0)->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback (&RxDrop)); 
          }
          ...
          ndn::AppHelper consumerHelper("ns3::ndn::RepoSyncDrop");
